// Code generated by go-swagger; DO NOT EDIT.

package global_network_manager_service

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/hashicorp/hcp-sdk-go/clients/cloud-global-network-manager-service/preview/2022-02-15/models"
	cloud "github.com/hashicorp/hcp-sdk-go/clients/cloud-shared/v1/models"
)

// ListClustersWithAcceptorEligibilityReader is a Reader for the ListClustersWithAcceptorEligibility structure.
type ListClustersWithAcceptorEligibilityReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *ListClustersWithAcceptorEligibilityReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewListClustersWithAcceptorEligibilityOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		result := NewListClustersWithAcceptorEligibilityDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewListClustersWithAcceptorEligibilityOK creates a ListClustersWithAcceptorEligibilityOK with default headers values
func NewListClustersWithAcceptorEligibilityOK() *ListClustersWithAcceptorEligibilityOK {
	return &ListClustersWithAcceptorEligibilityOK{}
}

/*
ListClustersWithAcceptorEligibilityOK describes a response with status code 200, with default header values.

A successful response.
*/
type ListClustersWithAcceptorEligibilityOK struct {
	Payload *models.HashicorpCloudGlobalNetworkManager20220215ListClustersWithAcceptorEligibilityResponse
}

// IsSuccess returns true when this list clusters with acceptor eligibility o k response has a 2xx status code
func (o *ListClustersWithAcceptorEligibilityOK) IsSuccess() bool {
	return true
}

// IsRedirect returns true when this list clusters with acceptor eligibility o k response has a 3xx status code
func (o *ListClustersWithAcceptorEligibilityOK) IsRedirect() bool {
	return false
}

// IsClientError returns true when this list clusters with acceptor eligibility o k response has a 4xx status code
func (o *ListClustersWithAcceptorEligibilityOK) IsClientError() bool {
	return false
}

// IsServerError returns true when this list clusters with acceptor eligibility o k response has a 5xx status code
func (o *ListClustersWithAcceptorEligibilityOK) IsServerError() bool {
	return false
}

// IsCode returns true when this list clusters with acceptor eligibility o k response a status code equal to that given
func (o *ListClustersWithAcceptorEligibilityOK) IsCode(code int) bool {
	return code == 200
}

// Code gets the status code for the list clusters with acceptor eligibility o k response
func (o *ListClustersWithAcceptorEligibilityOK) Code() int {
	return 200
}

func (o *ListClustersWithAcceptorEligibilityOK) Error() string {
	return fmt.Sprintf("[GET /global-network-manager/2022-02-15/organizations/{location.organization_id}/projects/{location.project_id}/clusters_with_acceptor_eligibility][%d] listClustersWithAcceptorEligibilityOK  %+v", 200, o.Payload)
}

func (o *ListClustersWithAcceptorEligibilityOK) String() string {
	return fmt.Sprintf("[GET /global-network-manager/2022-02-15/organizations/{location.organization_id}/projects/{location.project_id}/clusters_with_acceptor_eligibility][%d] listClustersWithAcceptorEligibilityOK  %+v", 200, o.Payload)
}

func (o *ListClustersWithAcceptorEligibilityOK) GetPayload() *models.HashicorpCloudGlobalNetworkManager20220215ListClustersWithAcceptorEligibilityResponse {
	return o.Payload
}

func (o *ListClustersWithAcceptorEligibilityOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.HashicorpCloudGlobalNetworkManager20220215ListClustersWithAcceptorEligibilityResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewListClustersWithAcceptorEligibilityDefault creates a ListClustersWithAcceptorEligibilityDefault with default headers values
func NewListClustersWithAcceptorEligibilityDefault(code int) *ListClustersWithAcceptorEligibilityDefault {
	return &ListClustersWithAcceptorEligibilityDefault{
		_statusCode: code,
	}
}

/*
ListClustersWithAcceptorEligibilityDefault describes a response with status code -1, with default header values.

An unexpected error response.
*/
type ListClustersWithAcceptorEligibilityDefault struct {
	_statusCode int

	Payload *cloud.GoogleRPCStatus
}

// IsSuccess returns true when this list clusters with acceptor eligibility default response has a 2xx status code
func (o *ListClustersWithAcceptorEligibilityDefault) IsSuccess() bool {
	return o._statusCode/100 == 2
}

// IsRedirect returns true when this list clusters with acceptor eligibility default response has a 3xx status code
func (o *ListClustersWithAcceptorEligibilityDefault) IsRedirect() bool {
	return o._statusCode/100 == 3
}

// IsClientError returns true when this list clusters with acceptor eligibility default response has a 4xx status code
func (o *ListClustersWithAcceptorEligibilityDefault) IsClientError() bool {
	return o._statusCode/100 == 4
}

// IsServerError returns true when this list clusters with acceptor eligibility default response has a 5xx status code
func (o *ListClustersWithAcceptorEligibilityDefault) IsServerError() bool {
	return o._statusCode/100 == 5
}

// IsCode returns true when this list clusters with acceptor eligibility default response a status code equal to that given
func (o *ListClustersWithAcceptorEligibilityDefault) IsCode(code int) bool {
	return o._statusCode == code
}

// Code gets the status code for the list clusters with acceptor eligibility default response
func (o *ListClustersWithAcceptorEligibilityDefault) Code() int {
	return o._statusCode
}

func (o *ListClustersWithAcceptorEligibilityDefault) Error() string {
	return fmt.Sprintf("[GET /global-network-manager/2022-02-15/organizations/{location.organization_id}/projects/{location.project_id}/clusters_with_acceptor_eligibility][%d] ListClustersWithAcceptorEligibility default  %+v", o._statusCode, o.Payload)
}

func (o *ListClustersWithAcceptorEligibilityDefault) String() string {
	return fmt.Sprintf("[GET /global-network-manager/2022-02-15/organizations/{location.organization_id}/projects/{location.project_id}/clusters_with_acceptor_eligibility][%d] ListClustersWithAcceptorEligibility default  %+v", o._statusCode, o.Payload)
}

func (o *ListClustersWithAcceptorEligibilityDefault) GetPayload() *cloud.GoogleRPCStatus {
	return o.Payload
}

func (o *ListClustersWithAcceptorEligibilityDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(cloud.GoogleRPCStatus)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
